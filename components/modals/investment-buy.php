    ...
    <div class="w-150 bg-slate-50 m-5 rounded-xl shadow-md">
      ...
      hover:text-slate-50 hover:bg-red-400 rounded-lg">
        ...
        <div
            ...
            <form
                action="include/investment/investment-buy.php?action=<?= $_GET["action"] ?>&id=<?= $_GET["id"] ?>&type=<?= $_GET["type"] ?>"
                method="POST" class="flex flex-col gap-6 h-full">
                            ...
                            <?php for ($i = 0; $i < count($accounts); $i++): ?>
                                ...
                                <el-option value="<?= $accounts[$i][0] ?>"
                                    ...
                                    <div class="flex flex-col w-full">
                                        ...
                                        <span
                                            class="font-bold text-left text-slate-950"><?= number_format($accounts[$i][2], 2, ',', ' ') ?>
                                            ₽</span>
                                    </div>
                                    <span
                ...
                <span class="flex flex-col gap-2">
                    <label class="font-bold text-slate-950 text-lg flex items-center gap-2" for="">Количество <p
                            id="securities-price" class="text-sm font-normal text-slate-700">(цена: 0 ₽)</p></label>
                    <input require id="securities-value" type="number"
