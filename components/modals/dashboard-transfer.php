    ...
    <div class="w-150 bg-slate-50 m-5 rounded-xl shadow-md">
      ...
      hover:text-slate-50 hover:bg-red-400 rounded-lg">
        ...
        <div
            ...
            <h1 class="font-bold text-3xl text-slate-950">Перевод на другой счёт</h1>

            <?php if (isset($_SESSION["account-transfer-error"])): ?>
                <div class="w-full bg-red-500/25 p-4 rounded-xl border-1 border-red-500 text-red-950">
                    <?= $_SESSION["account-transfer-error"] ?>
                    <?php unset($_SESSION["account-transfer-error"]) ?>
                </div>
            <?php endif ?>
            ...
            <div class="flex flex-col">
              ...
              hover:border-blue-500 hover:bg-blue-100 placeholder:text-state-400">
                            ...
                            <el-options anchor="bottom start" popover
                                ...
                                <el-option value="1"
                                    ...
                                    <div class="flex flex-col w-full">
                                        ...
                                        <span
                                            class="font-bold text-left text-slate-950"><?= number_format(get_user_balance($role["client_id"]), 2, ',', ' ') ?>
                                            ₽</span>
                                    </div>
                                    <span
                                        class="absolute inset-y-0 right-0 flex items-center pr-4 text-blue-500 group-not-aria-selected/option:hidden group-focus/option:text-blue-500 in-[el-selectedcontent]:hidden">
                    ...
                    <span class="flex flex-col gap-2">
                        <label class="font-bold text-slate-950 text-lg" for="account-id">Номер карты</label>
                        <input type="number"
                            class="p-4 bg-slate-100 outline-none border-1 border-slate-600 rounded-lg 
            hover:border-blue-500 hover:bg-blue-100 cursor-pointer placeholder:text-state-400 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            placeholder="0" step="0.01" max="1000000" name="account-id">
                    </span>

                    <span class="flex flex-col gap-2">
                        <label class="font-bold text-slate-950 text-lg" for="transfer-value">Сумма</label>
                        <input type="number"
                            class="p-4 bg-slate-100 outline-none border-1 border-slate-600 rounded-lg 
                            hover:border-blue-500 hover:bg-blue-100 cursor-pointer placeholder:text-state-400 
                            [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none 
                            [&::-webkit-inner-spin-button]:appearance-none"
                            placeholder="0 ₽" step="0.01" max="1000000" name="transfer-value">
                    </span>
